...
import { ArrowLeft, MapPin, Wheat, FileText, MessageCircle, Printer } from "lucide-react";
...
export default function ParcelReportPage() {
  ...
  useEffect(() => {
    if (selectedId) loadParcelData(selectedId);
  }, [selectedId]);
  ...
  const selectedParcel: any = parcels.find((p: any) => p.id === selectedId);
  ...
  const handlePDF = async () => {
    ...
    try {
      ...
      const rows = [
        ...
        ...Object.entries(expByCategory).map(([k, v]) => [`Expense - ${k}`, fmt(v)]),
      ];
  ...
  const handleWhatsApp = async () => {
    ...
    shareViaWhatsApp(
      `Parcel Report - ${selectedParcel?.name}`,
      ...
      <div className="px-4 pt-12 pb-5" style={{ backgroundColor: "#1B5E20" }}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-green-200 text-xs">Crop history & profitability</p>
            </div>
          </div>
          <button
            onClick={handlePrint}
        ...
        <div className="bg-white/20 rounded-2xl px-4 py-3 flex items-center gap-3">
          ...
          <select value={selectedId} onChange={(e) => setSelectedId(e.target.value)}
            ...
            {parcels.map((p: any) => (
              <option key={p.id} value={p.id} style={{ color: "#1B5E20" }}>{p.name} ({p.acres} acres)</option>
            ))}
          </select>
        </div>
      </div>
              ...
              <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-gray-50 rounded-xl p-2">
                    <p className="text-gray-400 text-xs">Acres</p>
                    <p className="font-bold text-gray-800">{selectedParcel.acres}</p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-2">
                    <p className="text-gray-400 text-xs">Location</p>
                    <p className="font-bold text-gray-800 text-sm">{selectedParcel.location || "—"}</p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-2 col-span-2">
                    <p className="text-gray-400 text-xs">Assigned Farmer</p>
                    <p className="font-bold text-gray-800">{selectedParcel.assignedFarmerName || "Not assigned"}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={handlePDF} disabled={exporting}
                  className="flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm active:scale-95 disabled:opacity-50"
                  style={{ backgroundColor: "#FFEBEE", color: "#C62828" }}>
                  <FileText size={16} /> PDF
                </button>
                <button onClick={handleWhatsApp}
                  className="flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm active:scale-95"
